...
def article_overview(kind, period):
    ...
    urls =[]
    for article in data:
        dict = {article['section']: article['title']}
        titles.append(dict)
        for info in article['media']:
            metadata = info['media-metadata']
            for metadatainfo in metadata:
                if metadatainfo['format'] == 'Standard Thumbnail':
                    urls.append(metadatainfo['url'])
    return titles, urls
