...
def process_file(filename, fields):
    ...
    classification_fields = ['family', 'class', 'phylum', 'order', 'kingdom', 'genus']
    data = []
        ...
        for line in reader:
            ...
            for field in process_fields:
                ...
                if field == 'rdf-schema#label' and element.find('(') != -1:
                    datainfo[FIELDS[field]] = element.split('(')[0].strip()
                elif field == 'name' and (element == "NULL" or element.isalnum() == False):
                    datainfo[FIELDS[field]] = datainfo['label']
                elif element == "NULL":
                    datainfo[FIELDS[field]] = None
                elif field == 'synonym':
                    datainfo[FIELDS[field]] = parse_array(element)
                else:
                    datainfo[FIELDS[field]] = element
            for field in classification_fields:
                classification[field] = datainfo.pop(field)
